...
"""Так и есть. Дескрипторы не нужны для простых классов. Их
...
переменные уровня класса, определённые сразу после заголовка класса Student.
...
попытке присвоения значения не проходящего валидацию получаем соответствующую ошибку.
...
class User:
    first_name = Text(str.istitle) # НЕ ПРОЙДЕТ ПРОВЕРКУ, ОШИБКА
    last_name = Text(str.isupper) # НЕ ПРОЙДЕТ ПРОВЕРКУ, ОШИБКА
...
# ПОЛЬЗОВАТЕЛЯ НЕ УДАСТСЯ СОЗДАТЬ, т.к. введенные значения атрибутов не соответствуют
# условиям, проверяемых дескрипторами
