...
def func(file_name):
    store_id = set()
    if os.path.exists(file_name): # если такой файл уже существует, открываем его на чтение, загружаем существующий словарь с данными и с ним дальше будем работать
        with open(file_name, 'r', encoding='utf-8') as f:
            new_dict = json.load(f)
            for _, value in new_dict.items():
                store_id.update(value.keys()) 
# если файл существует, значит там уже есть сет с сохраненными id, которые являются ключами вложенного словаря, апдейтим(выгружаем) их, чтобы дальше проверять уникальность следующих id
    else:   
