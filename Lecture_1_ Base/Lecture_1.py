...
# MATCH / CASE - множественное сравнение
...
# Определяем високосный ли год (по Григорианскому календарю): если остаток от деления года на 4 равен 0 или остаток от
# деления на 100 = 0 и остаток от деления на 400 = 0.
...
# ПРОВЕРКА НА ВХОЖДЕНИЕ элемента в последовательность (оператор IN)
...
# Перебираем все четные числа от 0 до введенного пользователем (исключительно)
...
# Выведем все четные числа, как в прошлом примере, кроме тех, которые кратны 12

# num = float(input('Enter number: '))
# STEP = 2 # добавили константу для движения по чётным и ушли от "магических чисел". Теперь изменение условия на "вывод чисел
# кратных 5" потребует замены числа в одном месте кода (step = 2 на step = 5).
...
# BREAK
# Пишем программу, которая просит ввести число в пределах заданного диапазона
...
# Зарезервированное слово else может применятся не только к ветвлениям, но и к циклам. Для этого else должно быть расположено
# на том же уровне, т.е. иметь столько же пробельных отступов, что и начало цикла — while.

# Пишем программу, которая просит ввести число в пределах заданного диапазона, но даем 3 попытки для попадания в диапазон
...
# команды continue, break и else могут применяться к циклу for in точно так же как и до этого в цикле while
