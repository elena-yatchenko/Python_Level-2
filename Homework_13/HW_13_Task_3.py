...
class InvalidNameError(ValueError):
    ...
    def __str__(self):
        return f"данные {self.value} не являются строкой или являются пустой строкой"
...
class InvalidAgeError(ValueError):
    ...
    def __str__(self):
        return f"данные {self.age} не являются целым положительным числом"
...
class InvalidIdError(ValueError):
    ...
    def __str__(self):
        return f"Введен некорректный ID: {self._id} (должно быть шестизначное целое положительное число)"


class Person:
    def __init__(self, surname, name, father_name, age):
        """другой вариант условия для пустой строки - len(last_name.strip()) == 0"""
        if surname == "" or not isinstance(surname, str):
            raise InvalidNameError(surname)
        if name == "" or not isinstance(name, str):
            raise InvalidNameError(name)
        if father_name == "" or not isinstance(father_name, str):
    ...
    def __repr__(self):
        return f"Person({self.surname}, {self.name}, {self.father_name}, {self._age})"
...
class Employee(Person):
    ...
    def __init__(self, _id, *args):
        """другой вариант проверки id: if not isinstance(id, int) or id < 100_000 or id > 999_999"""
        if not isinstance(_id, int) or _id < 0 or len(str(_id)) != 6:
            raise InvalidIdError(_id)
        self._id = _id
        super().__init__(*args)

    def __repr__(self):
        return f"Employee({self._id}, {self.surname}, {self.name}, {self.father_name}, {self._age})"

    def get_level(self):
        summa = sum((int(i) for i in str(self._id)))
        return summa % self.DEVIDER


e = Employee(124543, "454", "kkk", "dfkjgkdf", 21)
print(e)


p = Person("454", "kkk", "dfkjgkdf", 21)
p.birthday()
print(p._age)
e.birthday()
print(e._age)
...
#     def __init__(
#         self, last_name: str, first_name: str, patronymic: str, age: int, id: int
#     ):
...
# e = Employee("454", "kkk", "dfkjgkdf", 21, 124543)
