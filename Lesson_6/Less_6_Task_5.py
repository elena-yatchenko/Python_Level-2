...
# функция-угадайка с задачи 4 + формирование словаря закадка - попытка, с которой угадали
...
def fun_2(my_dict: dict) -> int:
    for key, value in my_dict.items():
        func(key, value) # попытки не указываем, берем значение по умолчанию
    show_result()
...
# в функции объединяем полученные генераторные объекты через join, соединяя переходом на новую строку
def show_result(): 
...
# Загадку "Зимой и летом одним цветом" угадали с 1 попытки
# Загадку Не лает, не кусает, в дом не пускает не угадали
# Загадку "Сидит дед во сто шуб одет" угадали с 3 попытки
